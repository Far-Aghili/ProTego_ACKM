...
def Receive_Vault(u_id, u_att, u_MkId, u_kek, P_F):#P_F was added to the policy file in this version
    ...
    print(x_KEK)
    p_dot3 = x_KEK.find('....')
    Group_IDs = x_KEK[p_dot3 + 4:len(x_KEK)]
    p_dot2 = x_KEK.find('...')
    KEK = x_KEK[p_dot2 + 3:p_dot3]
    p_dot1 = x_KEK.find('..')
    Grp = x_KEK[p_dot1 + 2:p_dot2]
    p_dot0 = x_KEK.find('.')
    Rl = x_KEK[p_dot0 + 1:p_dot1]
    id = x_KEK[:p_dot0]
    print(u_id, id, u_group, Grp, u_role, Rl, Group_IDs)
    KEK_decision = Receive_decision(u_id, id, u_group, Grp, u_role, Rl, P_F, Group_IDs)#P_F was added to the policy file in this version
    if KEK_decision == '1':
